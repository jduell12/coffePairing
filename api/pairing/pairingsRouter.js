...
const { addPairing, getPairingBy, deletePairingByDate } = require("./pairingsModel");
const { getFemmeBy } = require("../femmegineers/femmegineersModel");
...
router.post("/:month/:year", async (req, res) => {
  ...
  const db_pairs = await getPairingBy('month+year', [month, year])
  ...
  const femme_list = await getFemmeBy("active", true);
  //get previous pairings
  const prev_pair = await getPairingBy("year", year);
  ...
  for (let i = 0; i < pairs.length; i++) {
    await addPairing(pairs[i]);

    //get detailed femme by id to send to endpoint
    const femme1 = await getFemmeBy("femme_id", pairs[i].pair1);

    const femme2 = await getFemmeBy("femme_id", pairs[i].pair2);
...
router.get("/:month/:year", async (req, res) => {
  try {
    ...
    let pairings_to_send = []
    const pairings = await getPairingBy("month+year", [month, year])

    //get the detailed femme information for pairing
    for (p in pairings) {
      let pairing = pairings[p]
      const femme1 = await getFemmeBy('femme_id', pairing.pair1)

      const femme2 = await getFemmeBy("femme_id", pairing.pair2);
...
//delete a pairing for a given month and year
router.delete("/:month/:year", async (req, res) => {
  try {
    let { month, year } = req.params;

    await deletePairingByDate({ month, year })

    res.json({ message: `Successfully reset pairings for ${month} ${year}` })
